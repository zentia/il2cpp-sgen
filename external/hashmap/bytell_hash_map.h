...
struct sherwood_v8_constants
{
    ...
    static constexpr ulong_size_t jump_distances[num_jump_distances]
    {
...
template<typename T>
constexpr ulong_size_t sherwood_v8_constants<T>::jump_distances[num_jump_distances];
...
class sherwood_v8_table : private ByteAlloc, protected Hasher, private Equal
{
...
public:
    ...
    private:
        ...
        ulong_size_t index = 0;

    public:
        templated_iterator()
        {
        }
        templated_iterator(BlockPointer entries, ulong_size_t index)
            : current(entries)
            , index(index)
        {
        }
    ...
    iterator begin()
    {
        ulong_size_t num_slots = num_slots_minus_one ? num_slots_minus_one + 1 : 0;
    ...
    iterator end()
    {
        return { entries - 1, std::numeric_limits<ulong_size_t>::max() };
    }
    ...
    {
        ulong_size_t index = hash_object(key);
        ulong_size_t num_slots_minus_one = this->num_slots_minus_one;
        BlockPointer entries = this->entries;
        index = hash_policy.index_for_hash(index, num_slots_minus_one);
        bool first = true;
        for (;;)
        {
            ulong_size_t block_index = index / BlockSize;
    ...
    void resize(ulong_size_t num_items)
    {
        rehash(num_items);
    }
	
    void rehash(ulong_size_t num_items)
    {
        num_items = std::max(num_items, static_cast<ulong_size_t>(std::ceil(num_elements / static_cast<double>(_max_load_factor))));
        if (num_items == 0)
        {
            reset_to_empty_state();
            return;
        ...
        if (num_items == num_slots_minus_one + 1)
            return;
        ulong_size_t num_blocks = num_items / BlockSize;
        if (num_items % BlockSize)
            ++num_blocks;
        ulong_size_t memory_requirement = calculate_memory_requirement(num_blocks);
        ...
        if (num_items)
            ++num_items;
        ulong_size_t old_num_blocks = num_items / BlockSize;
        if (num_items % BlockSize)
            ++old_num_blocks;
    ...
    void clear()
    {
        if (!num_slots_minus_one)
            return;
        ulong_size_t num_slots = num_slots_minus_one + 1;
        ulong_size_t num_blocks = num_slots / BlockSize;
...
private:
    ...
    {
        return static_cast<ulong_size_t>(std::ceil(num_elements / static_cast<double>(_max_load_factor)));
    }
    ...
    struct LinkedListIt
    {
        ...
        LinkedListIt(ulong_size_t index, BlockPointer block)
        ...
        LinkedListIt next(sherwood_v8_table & table) const
        {
            ...
            ulong_size_t next_index = table.hash_policy.keep_in_range(index + Constants::jump_distances[distance], table.num_slots_minus_one);
    ...
    {
        ulong_size_t to_move_hash = hash_object(*child);
        ulong_size_t to_move_index = hash_policy.index_for_hash(to_move_hash, num_slots_minus_one);
    ...
    std::pair<int8_t, LinkedListIt> find_free_index(LinkedListIt parent) const
    {
        ...
        {
            ulong_size_t index = hash_policy.keep_in_range(parent.index + Constants::jump_distances[jump_index], num_slots_minus_one);
    ...
    void grow()
    {
        rehash(std::max(ulong_size_t(10), 2 * bucket_count()));
    }

    ulong_size_t calculate_memory_requirement(ulong_size_t num_blocks)
    {
        ulong_size_t memory_required = sizeof(BlockType) * num_blocks;
        memory_required += BlockSize; // for metadata of past-the-end pointer
        return memory_required;
    }

    void deallocate_data(BlockPointer begin, ulong_size_t num_slots_minus_one)
    {
        if (begin == BlockType::empty_block())
            return;

        ++num_slots_minus_one;
        ulong_size_t num_blocks = num_slots_minus_one / BlockSize;
        if (num_slots_minus_one % BlockSize)
            ++num_blocks;
        ulong_size_t memory = calculate_memory_requirement(num_blocks);
...
{
    static constexpr ulong_size_t value = 8;
};
template<typename T>
struct AlignmentOr8Bytes<T, typename std::enable_if<alignof(T) >= 1>::type>
{
    static constexpr ulong_size_t value = alignof(T);
};
template<typename... Args>
...
{
    static constexpr ulong_size_t this_value = AlignmentOr8Bytes<First>::value;
    static constexpr ulong_size_t base_value = CalculateBytellBlockSize<More...>::value;
    static constexpr ulong_size_t value = this_value > base_value ? this_value : base_value;
};
template<>
struct CalculateBytellBlockSize<>
{
...
class bytell_hash_set
        ...
        <
            ...
            T,
            H,
            detailv8::functor_storage<ulong_size_t, H>,
            E,
