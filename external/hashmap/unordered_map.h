...
public:
        ...
        *new_buckets = EntryPointer(nullptr) + ptrdiff_t(1);
        ...
        for (EntryPointer * it = new_buckets, * end = it + static_cast<ptrdiff_t>(num_buckets + 1); it != end; ++it)
        {
            for (EntryPointer e = *it; e;)
            {
                ...
                ulong_size_t index = hash_policy.index_for_hash(hash_object(e->value), num_slots_minus_one);
    ...
    void reserve(ulong_size_t num_elements)
    {
        if (!num_elements)
            return;
        num_elements = static_cast<ulong_size_t>(std::ceil(num_elements / static_cast<double>(_max_load_factor)));
...
class unordered_set
        ...
        <
            ...
            T,
            H,
            detailv10::functor_storage<ulong_size_t, H>,
            E,
