...
#else
# define MAX_BLACK_LIST_ALLOC (1000*HBLKSIZE)
 ...
 */
  ...
  #else
  #   define UNMAP_THRESHOLD 1//to unity2017 version
  #endif
#endif
...
GC_INNER void GC_merge_unmapped_v2(void)
{
    int i;
    ...
    for (i = 0; i <= N_HBLK_FLS; ++i) {
      ...
      while (h != 0) {
        ...
        hdr *hhdr, *nexthdr, *prevhdr;
        word size, nextsize, prevsize;
        GC_bool needNext = TRUE;

        GET_HDR(h, hhdr);
        size = hhdr->hb_sz;
        next = (struct hblk *)((word)h + size);
        prev = GC_free_block_ending_at(h);
        GET_HDR(next, nexthdr);
        /* Coalesce with successor, if possible */
          if (0 != nexthdr && HBLK_IS_FREE(nexthdr)
              && (signed_word) (size + (nextsize = nexthdr->hb_sz)) > 0
        ...
        if (0 != prev) {
            size = hhdr->hb_sz;
            GET_HDR(prev, prevhdr);
            if (0 != prevhdr && HBLK_IS_FREE(prevhdr) && (signed_word) (size + (prevsize = prevhdr->hb_sz)) > 0) {

                if (IS_MAPPED(hhdr) && !IS_MAPPED(prevhdr)) {
                    /* make both consistent, so that we can merge */
                    if (size > prevsize) {
                      GC_remap((ptr_t)prev, prevsize);
                    } else {
                      GC_unmap((ptr_t)h, size);
                      GC_unmap_gap((ptr_t)prev, prevsize, (ptr_t)h, size);
                      prevhdr -> hb_flags |= WAS_UNMAPPED;
                    }
                } else if (IS_MAPPED(prevhdr) && !IS_MAPPED(hhdr)) {
                    if (size > prevsize) {
                       GC_unmap((ptr_t)prev, prevsize);
                       GC_unmap_gap((ptr_t)prev, prevsize, (ptr_t)h, size);
                    } else {
                       GC_remap((ptr_t)h, size);
                       prevhdr -> hb_flags &= ~WAS_UNMAPPED;
                       prevhdr -> hb_last_reclaimed = hhdr -> hb_last_reclaimed;
                    }
                } else if (!IS_MAPPED(hhdr) && !IS_MAPPED(nexthdr)) {
                    /* Unmap any gap in the middle */
                    GC_unmap_gap((ptr_t)prevhdr, prevsize, (ptr_t)h, size);
                }
        ...
        if(needNext /* not mergable with successor */ ){
            h = hhdr -> hb_next;
        }
      } /* while (h != 0) ... */
    } /* for ... */
