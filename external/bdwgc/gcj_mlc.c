...
int GC_gcj_array_kind = 0;
GC_INNER ptr_t * GC_gcjarrayobjfreelist = NULL;
...
#   endif
    ...
    /* Set up object kind for objects that require mark proc call.      */
	  ...
	  /* Set up  array object kind indirect descriptor.      */
	  GC_gcjarrayobjfreelist = (ptr_t *)GC_new_free_list_inner();
	  GC_gcj_array_kind = GC_new_kind_inner(
		  (void **)GC_gcjarrayobjfreelist,
		  GC_DS_ARRAY,
		  FALSE, TRUE);
...
#if !IL2CPP_ENABLE_WRITE_BARRIER_VALIDATION
#ifdef THREAD_LOCAL_ALLOC
GC_INNER void * GC_core_gcj_malloc_array(size_t lb, void * ptr_to_struct_containing_descr)
#else
GC_API GC_ATTR_MALLOC void * GC_CALL GC_gcj_malloc_array(size_t lb, void * ptr_to_struct_containing_descr)
#endif
{
	ptr_t op;
	DCL_LOCK_STATE;
	...
	if (SMALL_OBJ(lb)) {
		...
		LOCK();
		lg = GC_size_map[lb];
		op = GC_gcjarrayobjfreelist[lg];
		if (EXPECT(0 == op, FALSE)) {
			maybe_finalize();
			op = (ptr_t)GENERAL_MALLOC_INNER((word)lb, GC_gcj_array_kind);
			if (0 == op) {
				GC_oom_func oom_fn = GC_oom_fn;
				UNLOCK();
				return((*oom_fn)(lb));
			...
			GC_gcjarrayobjfreelist[lg] = (ptr_t)obj_link(op);
			GC_bytes_allocd += GRANULES_TO_BYTES((word)lg);
		}
		GC_ASSERT(((void **)op)[1] == 0);
	}
