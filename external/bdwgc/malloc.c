...
GC_API GC_ATTR_MALLOC void * GC_CALL GC_malloc_static_uncollectable(size_t lb, size_t descr)
{
	lb = SIZET_SAT_ADD(lb, sizeof(size_t));
	word *op = (word *)GC_generic_malloc_uncollectable(lb, UNCOLLECTABLESTATIC);
	if (EXPECT(NULL == op, FALSE))
		return NULL;

	size_t lg = BYTES_TO_GRANULES(GC_size(op));
	op[GRANULES_TO_WORDS(lg) - 1] = descr;
