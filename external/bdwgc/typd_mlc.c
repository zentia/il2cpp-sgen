...
STATIC GC_word GC_add_bitmap_descriptor(const GC_fword * bm, word nbits)
{
	size_t bit_len = (nbits + GC_FWORDSZ) / GC_FWORDSZ;
	if (GC_bitmap_index + bit_len > GC_bitmap_len)
		return 0;

	#define GC_MAX_BITMAP_LEN 16//((size_t)1 << (GC_WORDSZ - GC_LOG_MAX_MARK_BITMAP_INDEX_LEN - GC_DS_TAG_BITS))
    if(bit_len > GC_MAX_BITMAP_LEN)
	{
		GC_err_printf("bit_len is overflow len:%d",bit_len);
		return 0;
	}
	
	DCL_LOCK_STATE;
    LOCK();
	//GC_log_printf("GC_add_bitmap_descriptor:%d bm:%lu",nbits,(bm)[0]);
	for (size_t i = 0; i < bit_len; i++)
	{
		GC_bitmap_descriptors[GC_bitmap_index + i] = bm[i];
	}

	GC_word descr = ((bit_len << (GC_LOG_MAX_MARK_BITMAP_INDEX_LEN + GC_DS_TAG_BITS)) | (GC_bitmap_index << GC_DS_TAG_BITS)) | GC_DS_BITMAP;
	//GC_log_printf("GC_add_bitmap_descriptor end:%lu bit_len:%d descr:%u ",(GC_bitmap_descriptors[GC_bitmap_index]), (int)bit_len,(int)descr);
	GC_bitmap_index += bit_len;
...
#   define OPT_THRESHOLD 50
	...
	else if (size <= BITMAP_BITS/2 && (descriptor & GC_DS_TAGS) != GC_DS_PROC && (size & (sizeof(word)-1)) == 0) {
      int result =
...
GC_API GC_descr GC_CALL GC_make_descriptor(const GC_fword * bm, size_t len)
{
...
//#   endif
    ...
    while (last_set_bit >= 0 && !GC_get_bit64(bm, last_set_bit))
      last_set_bit--;
...
#   endif
	...
	if ((word)last_set_bit < BITMAP_BITS_MINUS_ONE) 
	{
		result = (SIGNB | ((GC_fword)(*bm) << GC_DS_TAG_BITS)) | GC_DS_BITMAP;
	} 
	else 
	{
		result = GC_add_bitmap_descriptor(bm, (word)last_set_bit + 1);
		if (result == 0) return(WORDS_TO_BYTES(last_set_bit + 1) | GC_DS_LENGTH);
	}
	return result;
#endif // MAKE_BITMAP_DESC
}

GC_API GC_descr GC_CALL GC_make_sarray_descriptor()
{
    #ifndef MAKE_BITMAP_DESC
    return 0;
    #else
...
GC_API GC_ATTR_MALLOC void * GC_CALL GC_array_malloc(size_t lb)
{
#ifndef MAKE_BITMAP_DESC
	return 0;
#else
	return GC_malloc_kind(lb, GC_array_kind);
    /*
	ptr_t op;
	ptr_t * opp;
	size_t lg;
	...
	if (SMALL_OBJ(lb)) {
		GC_DBG_COLLECT_AT_MALLOC(lb);
		lg = GC_size_map[lb];
		opp = &(GC_obj_kinds[GC_array_kind].ok_freelist[lg]);
		LOCK();
		op = *opp;
		if (EXPECT(0 == op, FALSE)) {
			UNLOCK();
