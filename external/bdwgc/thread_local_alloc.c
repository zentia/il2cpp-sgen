...
#       ifdef GC_GCJ_SUPPORT
			...
			p -> gcjarr_freelists[j] = (void *)(word)1;
#       endif
    }
...
#   ifdef GC_GCJ_SUPPORT
		...
		return_freelists(p -> gcjarr_freelists, (void **)GC_gcjarrayobjfreelist);
#   endif
}
...
//GC_gcj_malloc_array
GC_API GC_ATTR_MALLOC void* GC_CALL GC_gcj_malloc_array(size_t bytes,void * ptr_to_struct_containing_descr)
{
  if (EXPECT(GC_incremental, FALSE)) {
    return GC_core_gcj_malloc_array(bytes,void * ptr_to_struct_containing_descr);
  } else {
    size_t granules = ROUNDED_UP_GRANULES(bytes);
    void *result;
    void **tiny_fl;

    GC_ASSERT(GC_gcjarr_malloc_initialized);
    tiny_fl = ((GC_tlfs)GC_getspecific(GC_thread_key))->gcjarr_freelists;
    GC_FAST_MALLOC_GRANS(result, granules, tiny_fl, DIRECT_GRANULES,
                         GC_gcj_kind_array,
                         GC_core_gcj_malloc_array(bytes,void * ptr_to_struct_containing_descr),
                         {AO_compiler_barrier();
                          *(void **)result = ptr_to_struct_containing_descr;});
    return result;
  }
...
#     ifdef GC_GCJ_SUPPORT
        if (EXPECT(j > 0, TRUE)) {
		  ...
		  q = (ptr_t)AO_load((volatile AO_t *)&p->gcjarr_freelists[j]);
          if ((word)q > HBLKSIZE)
            GC_set_fl_marks(q);
        }
#     endif
...
#         ifdef GC_GCJ_SUPPORT
			...
			GC_check_fl_marks(&p->gcjarr_freelists[j]);
#         endif
        }
    }
#endif /* GC_ASSERTIONS */
