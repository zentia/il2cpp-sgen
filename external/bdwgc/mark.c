...
/* UNCOLLECTABLE */
              { &GC_uobjfreelist[0], 0,
                ...
                /*, */ OK_DISCLAIM_INITZ },
/* UNCOLLECTABLESTATIC */
			  { &GC_suobjfreelist[0], 0,
				/* 0 | */ (WORDS_TO_BYTES((word)-1) | GC_DS_PER_OBJECT), TRUE /* add length to descr */, TRUE
				/*, */ OK_DISCLAIM_INITZ },
...
GC_INNER size_t GC_array_offset_data = 16;
GC_INNER size_t GC_array_offset_length = 12;
GC_INNER size_t GC_array_offset_element_class = 32;
GC_INNER size_t GC_array_offset_element_size = 144;
...
GC_API void GC_set_array_info(GC_fword* desc_array,int desc_array_len, int array_offset_data, int array_offset_length, int array_offset_element_class, int array_offset_element_size)
{
	//GC_log_printf("GC_bitmap_len:%d GC_array_offset_data:%d GC_array_offset_length:%d GC_array_offset_element_class:%d GC_array_offset_element_size:%d ", desc_array_len, array_offset_data,array_offset_length,array_offset_element_class,array_offset_element_size);
	GC_bitmap_descriptors = desc_array;
	GC_bitmap_len = desc_array_len;
    GC_bitmap_index = 0;
	GC_array_offset_data = array_offset_data;
	GC_array_offset_length = array_offset_length;
	
	GC_array_offset_element_class = array_offset_element_class;
	GC_array_offset_element_size = array_offset_element_size;
}
/*
...
GC_INNER int GC_make_from_bitmap_desc(ptr_t current_p, GC_word descr, mse **stack_top, mse **stack_limit, word descr_tag)
{
	if (GC_bitmap_len == 0)
		return GC_bitmap_len;

	//ptr_t greatest_ha = (ptr_t)GC_greatest_plausible_heap_addr;
	//ptr_t least_ha = (ptr_t)GC_least_plausible_heap_addr;
	//GC_log_printf("GC_make_from_bitmap_desc:%d %d %d", descr, (word)((ptr_t)GC_greatest_plausible_heap_addr), (word)GC_greatest_plausible_heap_addr);

	mse* mark_stack_top = *stack_top;
	mse* mark_stack_limit= *stack_limit;
	...
	if ((descr & SIGNB) != 0 && 0) 
	{
		...
		//need surpport static descr
		if(descr_tag == GC_DS_ARRAY)
		{
			descr >>= 2;//top containing klass and monitor pointer
		}
		
		while (descr != 0) 
		{
            if ((descr & (GC_word)1) != 0) 
			{
              word current = *(word *)current_p;
              FIXUP_POINTER(current);
              if (current >= (word)GC_least_plausible_heap_addr && current < (word)GC_greatest_plausible_heap_addr)
				{
		...
		*stack_top = mark_stack_top;
		*stack_limit = mark_stack_limit;
		return WORDS_TO_BYTES(WORDSZ/2); /* guess */
	}
	
	word bit_len = descr >> (GC_LOG_MAX_MARK_BITMAP_INDEX_LEN + GC_DS_TAG_BITS);
	word bit_index = (GC_word)(descr >> GC_DS_TAG_BITS) & ((1 << GC_LOG_MAX_MARK_BITMAP_INDEX_LEN) - 1);
	//GC_log_printf("GC_make_from_bitmap_desc:%p %d %d %d %lld",current_p, descr, bit_index, bit_len,(GC_bitmap_descriptors[bit_index]));
	for (word i = 0; i < bit_len; ++i)
	{
		GC_fword bitmap = (GC_bitmap_descriptors[bit_index + i]);
		int bitmapCnt = 0;
		if(i == 0)
		{
			//bitmapCnt = 2;
			//bitmap = bitmap >> bitmapCnt;//top containing klass and monitor pointer
			//if(descr_tag == GC_DS_ARRAY)
			//	current_p += sizeof(word) * bitmapCnt;
			
			//need surpport static descr
			if(descr_tag == GC_DS_ARRAY)
			{
				bitmapCnt = 2;
				bitmap >>= bitmapCnt;//top containing klass and monitor pointer
			}
		}
		
		while (bitmap != 0)
		{
			if ((bitmap & (GC_fword)1) != 0)
			{
				word current = *(word*)current_p;
				FIXUP_POINTER(current);
				if (current >= (word)GC_least_plausible_heap_addr && current < (word)GC_greatest_plausible_heap_addr)
				{
					//GC_log_printf("current:%p",current);
					PREFETCH((ptr_t)current);
					PUSH_CONTENTS((ptr_t)current, mark_stack_top, mark_stack_limit, current_p);
				}
			}
        ...
        if ((i < bit_len - 1) && (bitmapCnt < FWORDSZ))
			current_p += sizeof(word) * (FWORDSZ - bitmapCnt);
	}
	*stack_top = mark_stack_top;
	*stack_limit = mark_stack_limit;
	return (WORDS_TO_BYTES(WORDSZ / 2)) * bit_len; /* guess */
}


GC_INNER void GC_make_from_array_desc(ptr_t current_p, GC_word descr, mse **stack_top, mse **stack_limit, word descr_tag)
{
    if ((descr & GC_DS_TAGS) == 0) 
    {
        mse* mark_stack_top = *stack_top;
        mse* mark_stack_limit= *stack_limit;
        ...
        ptr_t limit = current_p + (word)descr;
        limit -= 3 * sizeof(word);
        while ((word)current_p <= (word)limit)
        {
            word current = *(word *)current_p;
            FIXUP_POINTER(current);
            PREFETCH(current_p + 4*CACHE_LINE_SIZE);
            if (current >= (word)GC_least_plausible_heap_addr && current < (word)GC_greatest_plausible_heap_addr)
            {
              PREFETCH((ptr_t)current);
              PUSH_CONTENTS((ptr_t)current, mark_stack_top, mark_stack_limit, current_p);
    ...
    {
        GC_make_from_bitmap_desc(current_p, descr, stack_top, stack_limit, descr_tag);
    } 
}
...
#else
		  credit -= WORDS_TO_BYTES(WORDSZ / 2); /* guess */
          GC_MAKE_FROM_BITMAP_DESC(current_p, current, descr, mark_stack_top, mark_stack_limit, tag);
		  //GC_make_from_bitmap_desc(current_p, descr, &mark_stack_top, &mark_stack_limit, tag);
...
#         endif /* ENABLE_TRACE */
		...
		case GC_DS_ARRAY:
			...
			int array_length = *(int *)(current_p + GC_array_offset_length);
			if (array_length <= 0)
				continue;
			
			ptr_t array_data = current_p + GC_array_offset_data;

			ptr_t array_type = *(ptr_t *)current_p;
			ptr_t elem_type = *(ptr_t *)(array_type + GC_array_offset_element_class);
			int elem_size = *(int *)(array_type + GC_array_offset_element_size);

			//signed_word ok_descriptor = GC_obj_kinds[GC_gcj_kind].ok_descriptor;
			descr = *(word *)(elem_type + MARK_DESCR_OFFSET);
			//GC_log_printf("current_p:%p array_length:%d descr:%d elem_size:%d ",current_p,array_length,descr,elem_size);
			for (int idx = 0; idx < array_length; ++idx)
			{
				current_p = array_data + idx * elem_size;
                //descr = element_descr;
				//GC_make_from_array_desc(current_p, descr, &mark_stack_top, &mark_stack_limit, tag);
                GC_MAKE_FROM_ARRAY_DESC(current_p, current, descr, mark_stack_top, mark_stack_limit, tag);
			}

			credit -= elem_size * array_length;
			continue;
#endif // !MAKE_BITMAP_DESC
...
#         endif
        ...
        if (((hhdr->hb_obj_kind) & (~1)) == UNCOLLECTABLE) {
            GC_push_marked(h, hhdr);
            break;
        }
#       ifdef ENABLE_DISCLAIM
