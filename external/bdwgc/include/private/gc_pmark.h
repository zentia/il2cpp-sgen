...
#define BITMAP_BITS_MINUS_ONE (BITMAP_BITS - 1)
...
//unsigned long long* _bitmap_descriptors;
//#define GC_bitmap_descriptors _bitmap_descriptors;
GC_EXTERN size_t GC_bitmap_len = 0;
GC_EXTERN size_t GC_bitmap_index = 0;
GC_EXTERN GC_fword* GC_bitmap_descriptors;

/* Number of mark stack entries to discard on overflow. */
#define GC_MARK_STACK_DISCARDS (INITIAL_MARK_STACK_SIZE/8)

#define GC_MAKE_FROM_ARRAY_DESC(current_p, current, descr, mark_stack_top, mark_stack_limit, tag) \
    do { \
        if ((descr & GC_DS_TAGS) == 0) { \
            ptr_t end = current_p + (word)descr; \
            end -= sizeof(word); \
            while ((word)current_p <= (word)end) { \
...
#define GC_MAKE_FROM_BITMAP_DESC(current_p, current, descr, mark_stack_top, mark_stack_limit, tag) \
    do { \
        if ((descr & SIGNB) != 0) { \
            word descr_sign = descr & (~SIGNB); \
            descr_sign >>= GC_DS_TAG_BITS; \
            if(tag == GC_DS_ARRAY) { \
                descr_sign >>= 2; \
            } \
            while (descr_sign != 0) { \
                if ((descr_sign & (GC_word)1) != 0) { \
                  current = *(word *)current_p; \
                  FIXUP_POINTER(current); \
                  if (current >= (word)least_ha && current < (word)greatest_ha) { \
                        PREFETCH((ptr_t)current); \
