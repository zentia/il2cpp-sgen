...
#define GC_FWORDSZ (8 * sizeof(GC_fword))
#define GC_get_bit64(bm, index) \
            (((bm)[(index) / GC_FWORDSZ] >> ((index) % GC_FWORDSZ)) & 1)
#define GC_set_bit64(bm, index) \
            ((bm)[(index) / GC_FWORDSZ] |= (GC_fword)1 << ((index) % GC_FWORDSZ))
...
GC_API GC_descr GC_CALL GC_make_descriptor(const GC_fword * /* GC_bitmap bm */,
...
GC_API void GC_CALL GC_init_explicit_typing();
