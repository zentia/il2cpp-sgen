...
#include "Baselib_Custom_Allocator.h"

namespace baselib
{
    BASELIB_CPP_INTERFACE
    {
        // std library compatible deleter functior that uses Baselib_Internal_Memory_Free.
        template<typename T>
        struct AllocDeleter
        {
            ...
            {
                pointer->~T();
                ::Baselib_Internal_Memory_Free(pointer);
            }
        };

        // unique_ptr but using Baselib_Internal_Memory_Allocate.
        template<typename T>
        ...
        inline unique_ptr<T> make_unique(Args&& ... args)
        {
            ...
            if (alignof(T) <= Baselib_Memory_MinGuaranteedAlignment)
                memory = ::Baselib_Internal_Memory_Allocate(sizeof(T));
            else
                memory = ::Baselib_Internal_Memory_AlignedAllocate(sizeof(T), alignof(T));
            return unique_ptr<T>(new(memory) T(std::forward<Args>(args)...));
        }

        // std library compatible deleter functior that uses Baselib_Internal_Memory_AlignedFree.
        template<typename T>
        struct AlignedAllocDeleter
        {
            void operator()(T* pointer)
            {
                ...
                if (alignof(T) <= Baselib_Memory_MinGuaranteedAlignment)
                    ...
                    ::Baselib_Internal_Memory_AlignedFree(pointer);
            }
        };

        // A STL allocator using Baselib_Internal_Memory_Allocate
        template<typename T>
        class STLAllocator
        {
        public:
