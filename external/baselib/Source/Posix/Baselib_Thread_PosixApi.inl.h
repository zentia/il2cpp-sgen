...
BASELIB_C_INTERFACE
{
    ...
    {
        Baselib_Thread(Baselib_Thread_EntryPointFunction threadEntryPoint, void* threadEntryPointArgument, const char* threadName)
        ...
        TruncatedString<Baselib_Thread_MaxThreadNameLength> name;
        pthread_t handle;
    };
}
...
#define PTHREAD_STACK_MIN 16384
    ...
    BASELIB_INLINE_IMPL Baselib_Thread* Baselib_Thread_Create(Baselib_Thread_Config config, Baselib_ErrorState* errorState)
    {
        ...
        Baselib_Thread* thread = new(Baselib_Internal_Memory_Allocate(sizeof(Baselib_Thread)))
            Baselib_Thread(config.entryPoint, config.entryPointArgument, config.name ? config.name : "baselib_thread");
