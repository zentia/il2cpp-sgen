...
namespace detail
{
    ...
    BASELIB_STATIC_ASSERT(Baselib_SystemSemaphore_PlatformSize >= sizeof(detail::Semaphore), "Baselib_SystemSemaphore_PlatformSize must be same size or greater than internal representation");

    BASELIB_INLINE_IMPL Baselib_SystemSemaphore_Handle Baselib_SystemSemaphore_CreateInplace(void* semaphoreData)
    {
        detail::Semaphore* semaphore = static_cast<detail::Semaphore*>(semaphoreData);
    ...
    {
        detail::Semaphore* semaphore = static_cast<detail::Semaphore*>(Baselib_Internal_Memory_Allocate(sizeof(detail::Semaphore)));
        return Baselib_SystemSemaphore_CreateInplace(semaphore);
    }
    ...
    BASELIB_INLINE_IMPL void Baselib_SystemSemaphore_FreeInplace(Baselib_SystemSemaphore_Handle _semaphore)
    {
        detail::Semaphore* semaphore = ::detail::AsNativeType<detail::Semaphore*>(_semaphore);
    ...
    {
        DarwinApi::Baselib_SystemSemaphore_FreeInplace(_semaphore);
        Baselib_Internal_Memory_Free(::detail::AsNativeType<detail::Semaphore*>(_semaphore));
    }
}
