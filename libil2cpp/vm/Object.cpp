...
#include "vm/InsightGCStackTrace.h"
#endif

#if IL2CPP_GC_BOEHM
#define ALLOC_PTRFREE(obj, vt, size) do { (obj) = (Il2CppObject*)GC_MALLOC_ATOMIC ((size)); BIND_MALLOC(obj,size,true); (obj)->klass = (vt); (obj)->monitor = NULL;} while (0)
#define ALLOC_OBJECT(obj, vt, size) do { (obj) = (Il2CppObject*)GC_MALLOC ((size)); BIND_MALLOC(obj,size,true); (obj)->klass = (vt);} while (0)
#ifdef GC_GCJ_SUPPORT
#define ALLOC_TYPED(dest, size, type) do { (dest) = (Il2CppObject*)GC_gcj_malloc ((size),(type)); BIND_MALLOC(dest,size,true);} while (0)
#define ALLOC_ARRAY(dest, size, type) do { (dest) = (Il2CppObject*)GC_gcj_malloc_array ((size),(type)); BIND_MALLOC(dest,size,true);} while (0)
#else
#define GC_NO_DESCRIPTOR (NULL)
#define ALLOC_TYPED(dest, size, type) do { (dest) = GC_MALLOC ((size)); *(void**)dest = (type); BIND_MALLOC(dest,size,true);} while (0)
#define ALLOC_ARRAY(dest, size, type) do { (dest) = GC_MALLOC ((size)); *(void**)dest = (type); BIND_MALLOC(dest,size,true);} while (0)
#endif
#else
#ifdef HAVE_SGEN_GC
#define GC_NO_DESCRIPTOR (NULL)
...
#define ALLOC_ARRAY(dest, size, type) do { (dest) = mono_gc_alloc_obj (type, size);} while (0)
#else
...
#define ALLOC_ARRAY(dest, size, type) do { (dest) = (Il2CppObject*)calloc (1, (size)); *(void**)dest = (type);} while (0)
#endif
#endif
...
namespace vm
{
    Il2CppObject * Object::Allocate(size_t size, Il2CppClass *typeInfo)
    {
		...
		//LOGD("Allocate:%s %p", typeInfo->name,o);
        return o;
    }
    ...
    Il2CppObject * Object::AllocateSpec(size_t size, Il2CppClass *typeInfo)
    {
        ...
        ++il2cpp_runtime_stats.new_object_count;
		//LOGD("AllocateSpec:%s %p", typeInfo->name,o);
        return o;
    }

	Il2CppObject * Object::AllocateArray(size_t size, Il2CppClass *typeInfo)
	{
		
		IL2CPP_ASSERT(typeInfo->initialized);
		Il2CppObject *o;
		ALLOC_ARRAY(o, size, typeInfo);//ALLOC_TYPED(o, size, typeInfo);//

		++il2cpp_runtime_stats.new_object_count;
		//LOGD("AllocateArray:%s %p", typeInfo->name,o);
		return o;
	}
    ...
    {
#if ENABLE_GC_PROFILE
        il2cpp::vm::InsightGCStackTrace::InsightGCProfilerInnerSentry _sentry((int)il2cpp::vm::InsightGCStackTrace::ALLOCATION_OBJECT_BOX);
#endif
    ...
    {
#if ENABLE_GC_PROFILE
        il2cpp::vm::InsightGCStackTrace::InsightGCProfilerInnerSentry _sentry((int)il2cpp::vm::InsightGCStackTrace::ALLOCATION_OBJECT_CLONE);
#endif
        Il2CppObject *o;
        int size;
		...
		/*
        if (!obj->klass->has_references)
        {
            o = NewPtrFree(obj->klass);
        }
#if IL2CPP_HAS_GC_DESCRIPTORS
        else if (obj->klass->gc_desc != GC_NO_DESCRIPTOR)
        {
            o = AllocateSpec(size, obj->klass);
        }
#endif
        else
        {
...
#if ENABLE_GC_PROFILE
        Profiler::Allocation(o, obj->klass, size);

#elif IL2CPP_ENABLE_PROFILER
...
#error Invalid architecture size
    ...
    const MethodInfo* Object::GetVirtualMethod(Il2CppObject *obj, const MethodInfo *virtualMethod)
    {
        ...
        if (!obj->klass->is_vtable_initialized)
            il2cpp::vm::Class::SetupVTable(obj->klass);
    ...
    {
#if ENABLE_GC_PROFILE
        il2cpp::vm::InsightGCStackTrace::InsightGCProfilerInnerSentry _sentry((int)il2cpp::vm::InsightGCStackTrace::ALLOCATION_OBJECT_NEWSPECIFIC);
#endif
        Il2CppObject *o = NULL;
...
#if ENABLE_GC_PROFILE
        Profiler::Allocation(o, klass, klass->instance_size);
