...
namespace vm
{
    ...
    */
    ...
    struct LivenessState
    {
        ...
        static bool TraverseGCDescriptor(Il2CppObject* object, LivenessState* state, size_t obj_desc, bool isArray);
        static bool TraverseObjectInternal(Il2CppObject* object, bool isStruct, Il2CppClass* klass, LivenessState* state);
        static void TraverseArrayGCDescriptor(Il2CppArray* array, LivenessState* state);
...
// if all_objects run out of space, run through list, add objects that match the filter, clear bit in vtable and then clear the array.
    ...
    {
        Il2CppObject* filtered_objects[256];
        ...
        while (value)
        {
            ...
            if (num_objects == 256)
            {
                filter_callback(filtered_objects, 256, callback_userdata);
                num_objects = 0;
            }
    ...
    void LivenessState::TraverseGenericObject(Il2CppObject* object, LivenessState* state)
    {
        ...
        size_t gc_desc = (size_t)(klass->gc_desc);

        if ((gc_desc & 0x1))//have desc bitmap
            TraverseGCDescriptor(object, state, gc_desc, false);
        else
#endif
        {
            if (klass->rank)
            {
#if IL2CPP_HAS_GC_DESCRIPTORS
                TraverseArrayGCDescriptor((Il2CppArray*)object, state);
    ...
    bool LivenessState::TraverseGCDescriptor(Il2CppObject* object, LivenessState* state, size_t obj_desc, bool isArray)
    {
        bool added_objects = false;
#define WORDSIGNB  ((size_t)1 << (8 * sizeof(size_t) - 1))
        if ((obj_desc & WORDSIGNB) != 0)
        {
            int descIdx = 0;
            if (!isArray)
                descIdx = 2;
            ...
            obj_desc &= ~WORDSIGNB;
            obj_desc >>= 4;//2bit flag + 2bit top containing klass and monitor pointer

            while (obj_desc != 0)
            {
                if (obj_desc & 0x1)
                {
                    Il2CppObject* val = *(Il2CppObject**)(((char*)object) + descIdx * sizeof(void*));
        ...
        //   desclen|descindex                     |tagtype  
        //int bitmapCnt = obj_desc >> 26;
        //int bitmapIdx = (obj_desc >> 2) & (0xFFFFFF);

        //new mark mode
        //   0|000 0000 |0000 0000 0000 0000 0000 00|00
        //flag|desclen  |descindex                  |tagtype
        int bitmapCnt = obj_desc >> 24;
        int bitmapIdx = (obj_desc >> 2) & (0x3FFFFF);

        DWord* descTable = vm::MetadataCache::GetDescTablePointer();
        //LOGD("object:%p obj_desc:%d bitmapCnt:%d bitmapIdx:%d",object,obj_desc,bitmapCnt,bitmapIdx);
#define WORDSIZE ((int)sizeof(DWord)*8)
        for (int idx = 0; idx < bitmapCnt; idx++)
        {
            int descIdx = 0;
            const int headOffset = 2;//top containing klass and monitor pointer
            DWord descMask = descTable[bitmapIdx + idx];
            if (idx == 0)
            {
                if (!isArray)
                    descIdx = headOffset;
            ...
            while (descMask != 0)
            {
                if (descMask & 0x1)
                {
                    Il2CppObject* val = *(Il2CppObject**)(((char*)object) + (idx * WORDSIZE + descIdx) * sizeof(void*));
                    added_objects |= AddProcessObject(val, state);
                }
                descMask >>= 1;
                ++descIdx;
    ...
    void LivenessState::TraverseArray(Il2CppArray* array, LivenessState* state)
    {
        ...
        has_references = GET_CLASS(object)->has_references;// !Class::IsValuetype(element_class);
	...
	void LivenessState::TraverseArrayGCDescriptor(Il2CppArray* array, LivenessState* state)
    {
        Il2CppObject* object = (Il2CppObject*)array;
        Il2CppClass* element_class = GET_CLASS(object)->element_class;

        bool has_references = GET_CLASS(object)->has_references; //!Class::IsValuetype(element_class);
        ...
        //PtrFree for example int[]
        if (!has_references)
            return;

        size_t idx = 0;
        size_t arrayLen = Array::GetLength(array);// array->max_length;
        size_t gc_desc = (size_t)element_class->gc_desc;
        if (element_class->byval_arg.valuetype)
        {
            if (gc_desc & 0x1)//have desc bitmap
            {
                size_t items_processed = 0;
                int32_t elementSize = Class::GetArrayElementSize(element_class);//element_class->element_size;//
                for (idx = 0; idx < arrayLen; idx++)
                {
                    Il2CppObject* object = (Il2CppObject*)il2cpp_array_addr_with_size(array, elementSize, idx);
                    if (TraverseGCDescriptor(object, state, gc_desc, true))//(TraverseObjectInternal(object, 1, element_class, state))
                        items_processed++;
            ...
            else
            {
                ...
                for (idx = 0; idx < arrayLen; idx++)
                {
                    Il2CppObject* object = (Il2CppObject*)il2cpp_array_addr_with_size(array, (int32_t)elementSize, idx);
                    if (TraverseObjectInternal(object, 1, element_class, state))
                        items_processed++;
        ...
        else
        {
            ...
            for (idx = 0; idx < arrayLen; idx++)
            {
                Il2CppObject* val = il2cpp_array_get(array, Il2CppObject*, idx);
                if (AddProcessObject(val, state))
                    items_processed++;
