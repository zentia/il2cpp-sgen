...
namespace vm
{
    ...
    {
#if ENABLE_GC_PROFILE
        il2cpp::vm::InsightGCStackTrace::InsightGCProfilerInnerSentry _sentry((int)il2cpp::vm::InsightGCStackTrace::ALLOCATION_ARRAY_NEW);
#endif
    ...
    {
#if ENABLE_GC_PROFILE
        il2cpp::vm::InsightGCStackTrace::InsightGCProfilerInnerSentry _sentry((int)il2cpp::vm::InsightGCStackTrace::ALLOCATION_ARRAY_NEWSPECIFIC);
#endif
        Il2CppObject *o;
        Il2CppArray *ao;
        uint32_t elem_size;
...
#if !RUNTIME_TINY && ENABLE_GC_VECTOR_MALLOC
		...
		else if (klass->element_class->byval_arg.valuetype && klass->element_class->gc_desc != GC_NO_DESCRIPTOR)// class is reference by a pointer
		{
			//LOGD("Il2CppArrayLen:%d elem_size:%d",n,elem_size);
			//o = Object::Allocate(byte_len, klass);
			o = Object::AllocateArray(byte_len, klass);
		}
#endif
        else
        {
...
#if ENABLE_GC_PROFILE
        Profiler::Allocation(o, klass, byte_len);
    ...
    {
#if ENABLE_GC_PROFILE
        il2cpp::vm::InsightGCStackTrace::InsightGCProfilerInnerSentry _sentry((int)il2cpp::vm::InsightGCStackTrace::ALLOCATION_ARRAY_NEWFULL);
#endif
         ...
         */
		//LOGD("NewFull:%s gc_desc:%d valuetype:%d n:%d",array_class->name, array_class->element_class->gc_desc,array_class->element_class->valuetype,len);
        if (!array_class->has_references || len == 0)
        {
...
#if IL2CPP_HAS_GC_DESCRIPTORS
        else if (array_class->element_class->byval_arg.valuetype && array_class->element_class->gc_desc != GC_NO_DESCRIPTOR)
        {
			//o = Object::Allocate(byte_len, array_class);
            o = Object::AllocateArray(byte_len, array_class);
        }
#endif
        else
        {
...
#if ENABLE_GC_PROFILE
        Profiler::Allocation(o, array_class, byte_len);
