...
extern bool g_class_full_setup_mode;
...
namespace vm
{
    ...
    static void GetBitmapNoInit(Il2CppClass* klass, DWord* bitmap, size_t& maxSetBit, size_t parentOffset, const il2cpp::os::FastAutoLock* lockPtr);
    static void GetStaticBitmap(Il2CppClass* klass, DWord* bitmap, size_t& maxSetBit, size_t parentOffset, const il2cpp::os::FastAutoLock* lockPtr);
    ...
    static void GetMethodInfoFromMetadata(Il2CppClass *klass, const Il2CppMetadataMethodInfo& methodInfo, MethodInfo* newMethod);
...
#undef RETURN_DEFAULT_TYPE
        ...
        if (klass->generic_class != NULL)
        {
            ...
            for (Il2CppClass* iter = oklass; iter != NULL; iter = iter->parent)
            {
                ...
                if (!iter->is_vtable_initialized)
                    il2cpp::vm::Class::SetupVTable(iter);
    ...
    static void LayoutFieldsLocked(Il2CppClass *klass, const il2cpp::os::FastAutoLock& lock)
    {
        ...
        {
			const size_t kMaxAllocaSize = 128;
			size_t maxBits = klass->static_fields_size / sizeof(void*);
			size_t bitmapSize = ((size_t)((maxBits / 64) + 1)) * sizeof(DWord);

			DWord* bitmap = NULL;
			std::vector<DWord> buffer(0);
			if (bitmapSize > kMaxAllocaSize)
			{
				buffer.resize(bitmapSize / sizeof(DWord));
				bitmap = (DWord*)&buffer[0];
			}
			else
			{
				bitmap = (DWord*)alloca(bitmapSize);
			}
			memset(bitmap, 0, bitmapSize);

			size_t maxSetBit = 0;
			GetStaticBitmap(klass, bitmap, maxSetBit, 0, NULL);

			void* gc_desc = il2cpp::gc::GarbageCollector::MakeDescriptorForObject((DWord*)bitmap, (int)maxSetBit + 1);
			klass->static_fields = il2cpp::gc::GarbageCollector::AllocateStaticFixed(klass->static_fields_size, gc_desc);
			//LOGD("static_fields2:%p gc_desc:%d bitmap:%lld", klass->static_fields,(size_t)gc_desc, bitmap[0]);
...
// passing lock to ensure we have acquired it. We can add asserts later
    ...
    void SetupVirtualMethodsLocked(Il2CppClass *klass, const il2cpp::os::FastAutoLock& lock)
    {
        if (klass->is_methods_initialized || klass->is_virtual_method_initialized)
            return;

        klass->is_virtual_method_initialized = 1;
        ...
        else
        {
            ...
            uint16_t virtual_method_count = 0;
            for (MethodIndex index = start; index < end; ++index)
            {
                Il2CppMetadataMethodInfo methodInfo = MetadataCache::GetMethodInfo(klass, index - start);
                if (methodInfo.flags & METHOD_ATTRIBUTE_VIRTUAL)
                    ++virtual_method_count;
            }

            MethodInfo* methods = (MethodInfo*)MetadataCalloc(virtual_method_count, sizeof(MethodInfo));
            MethodInfo* newMethod = methods;

            int32_t count = 0;
            for (MethodIndex index = start; count < virtual_method_count && index < end; ++index)
            {
                Il2CppMetadataMethodInfo methodInfo = MetadataCache::GetMethodInfo(klass, index - start);

                if ((methodInfo.flags & METHOD_ATTRIBUTE_VIRTUAL) == 0)
                    continue;

                GetMethodInfoFromMetadata(klass, methodInfo, newMethod);
...
// passing lock to ensure we have acquired it. We can add asserts later
    ...
    {
        if ((!klass->method_count && !klass->rank) || klass->is_methods_initialized)
            return;

        klass->is_methods_initialized = 1;
        if (klass->has_cctor)
            klass->is_cctor_initialized = 1;
        ...
        else
        {
            ...
            uint16_t filled_method_count = 0;
            if (klass->methods == NULL)
            {
                klass->methods = (const MethodInfo**)MetadataCalloc(klass->method_count, sizeof(MethodInfo*));
            }
            ...
            if(klass->method_count == filled_method_count)
                return;

            MethodInfo* methods = (MethodInfo*)MetadataCalloc(klass->method_count - filled_method_count, sizeof(MethodInfo));
            MethodInfo* newMethod = methods;
    ...
    static void GetMethodInfoFromMetadata(Il2CppClass *klass, const Il2CppMetadataMethodInfo& methodInfo, MethodInfo* newMethod)
    {
        //keep indent to make later incoming changes happy
        {
            {
    ...
    void Class::SetupVirtualMethods(Il2CppClass *klass)
    {
        if (klass->method_count || klass->rank)
        {
            il2cpp::os::FastAutoLock lock(&g_MetadataLock);
            SetupVirtualMethodsLocked(klass, lock);
        }
    }
    ...
    static void SetupVTableLocked(Il2CppClass *klass, const il2cpp::os::FastAutoLock& lock)
    {
        ...
        {
            Il2CppClass* genericTypeDefinition = GenericClass::GetTypeDefinition(klass->generic_class);
            SetupVTableLocked(genericTypeDefinition, lock);
            Il2CppGenericContext* context = &klass->generic_class->context;
        ...
        else
        {
            ...
            if (klass->vtable_count > 0)
            {
                ...
                {
                    const MethodInfo* method = MetadataCache::GetVirtualMethodInfoFromVTableSlot(klass, i);
                    klass->vtable[i].method = method;
    ...
    static void SetupPropertiesLocked(Il2CppClass *klass, const il2cpp::os::FastAutoLock& lock)
    {
        ...
        {
            Il2CppClass* genericTypeDefinition = GenericClass::GetTypeDefinition(klass->generic_class);
            Class::InitLocked(genericTypeDefinition, lock);
            SetupPropertiesLocked(genericTypeDefinition, lock);
    ...
    static void SetupCCtorMethodsLocked(Il2CppClass *klass, const il2cpp::os::FastAutoLock& lock)
    {
        if (!klass->has_cctor || klass->is_cctor_initialized)
            return;
        ...
        {
            Class::InitLocked(GenericClass::GetTypeDefinition(klass->generic_class), lock);
            GenericClass::SetupCCtorMethods(klass);
        }
        else if (klass->rank)
        {
            Class::InitLocked(klass->element_class, lock);
        ...
        else
        {
            ...
            for (MethodIndex index = end - 1; index >= 0; --index)
            {
                Il2CppMetadataMethodInfo methodInfo = MetadataCache::GetMethodInfo(klass, index);

                if ((methodInfo.flags & METHOD_ATTRIBUTE_SPECIAL_NAME) &&
                    !strcmp(".cctor", methodInfo.name))
                {
                    GetMethodInfoFromMetadata(klass, methodInfo, newMethod);
    ...
    void Class::SetupCCtorMethods(Il2CppClass *klass)
    {
        if (klass->method_count || klass->rank)
        {
        	il2cpp::os::FastAutoLock lock(&g_MetadataLock);
        	SetupCCtorMethodsLocked(klass, lock);
        }
    }
    ...
    bool Class::InitLocked(Il2CppClass *klass, const il2cpp::os::FastAutoLock& lock)
    {
        ...
        if (g_class_full_setup_mode)
            SetupMethodsLocked(klass, lock);

        SetupTypeHierarchyLocked(klass, lock);

        if (klass == il2cpp_defaults.object_class || klass->has_finalize ||g_class_full_setup_mode)
            SetupVTableLocked(klass, lock);
        ...
        if (canBeInstantiated)
        {
            ...
            if (klass->generic_class)
            {
                ...
                if (klass->genericRecursionDepth < il2cpp::metadata::GenericMetadata::GetMaximumRuntimeGenericDepth() || il2cpp::vm::Runtime::IsLazyRGCTXInflationEnabled())
                {
                    ...
                    RGCTXCollection collection = MetadataCache::GetRGCTXs(klass->image, klass->token);
                    klass->rgctx_data = (Il2CppRGCTXData*)MetadataCalloc(collection.count, sizeof(Il2CppRGCTXData));
                }
            }
        }
    ...
    void Class::InitRGCTXLocked(Il2CppRGCTXData* rgctx_data, const MethodInfo* method, int index)
    {
        il2cpp::os::FastAutoLock lock(&g_MetadataLock);

        const Il2CppGenericMethod* gmethod = method->genericMethod;
        il2cpp::metadata::GenericMetadata::InflateOneRGCTXLocked(rgctx_data, gmethod->methodDefinition->klass->image, gmethod->methodDefinition->token, index,  &(gmethod->context), lock);
    }

    void Class::InitRGCTXLocked(Il2CppRGCTXData* rgctx_data, const Il2CppClass* klass, int index)
    {
        il2cpp::os::FastAutoLock lock(&g_MetadataLock);
        ...
        {
            if (klass->genericRecursionDepth < il2cpp::metadata::GenericMetadata::GetMaximumRuntimeGenericDepth() || il2cpp::vm::Runtime::IsLazyRGCTXInflationEnabled())
                il2cpp::metadata::GenericMetadata::InflateOneRGCTXLocked(rgctx_data, klass->image, klass->token, index, &klass->generic_class->context, lock);
        }
    }
    ...
    const size_t kDWordSize = (8 * sizeof(DWord));

    static inline void set_bit(DWord* bitmap, size_t index)
    {
        (bitmap[index / kDWordSize]) |= (DWord)1 << (index % kDWordSize);
    }

    size_t Class::GetBitmapSize(const Il2CppClass* klass)
    {
        size_t maxBits = klass->instance_size / sizeof(void*);
        size_t maxWords = 1 + (maxBits / kDWordSize);//sizeof(size_t)
        return sizeof(DWord) * maxWords;//sizeof(size_t)
    }

    void Class::GetBitmap(Il2CppClass* klass, DWord* bitmap, size_t& maxSetBit)
    {
        Class::Init(klass);
	...
	void GetStaticBitmap(Il2CppClass* klass, DWord* bitmap, size_t& maxSetBit, size_t parentOffset, const il2cpp::os::FastAutoLock* lockPtr)
    {
        ...
        if (currentClass)
        {
            for (uint16_t index = 0; index < currentClass->field_count; index++)
            {
                ...
                case IL2CPP_TYPE_MVAR:
                    ...
                    //LOGD("bitmap name:%s offset:%d",klass->name, offset);
                    break;
                case IL2CPP_TYPE_GENERICINST:
                ...
                case IL2CPP_TYPE_VALUETYPE:
                {
                    //LOGD("IL2CPP_TYPE_VALUETYPE:%s offset:%d",klass->name, offset);
                    ...
                    if (fieldClass->has_references)
                        GetBitmapNoInit(fieldClass, bitmap, maxSetBit, offset - sizeof(Il2CppObject), lockPtr);
                    break;
                }
                default:
    ...
    void GetBitmapNoInit(Il2CppClass* klass, DWord* bitmap, size_t& maxSetBit, size_t parentOffset, const il2cpp::os::FastAutoLock* lockPtr)
    {
        Il2CppClass* currentClass = klass;
    ...
    void SetupGCDescriptor(Il2CppClass* klass, const il2cpp::os::FastAutoLock& lock)
    {
        if (klass == il2cpp_defaults.string_class)
        {
            ...
            klass->gc_desc = 0;//need init value
            return;
        }
        ...
        const size_t kMaxAllocaSize = 64;//64 Byte Old:1024 bit
        ...
        {
            klass->gc_desc = 0;
            //LOGD("warnning name:%s bitmap:%lld maxSetBit:%d",klass->name, bitmap[0], (int)maxSetBit);
        }
        else
        {
            klass->gc_desc = il2cpp::gc::GarbageCollector::MakeDescriptorForObject((DWord*)bitmap, (int)maxSetBit + 1);
            //LOGD("make_desc: %s gc_desc:%d bitmap:%lld", klass->name, klass->gc_desc,bitmap[0]);
        }
    }
