...
namespace vm
{
    ...
    typedef utils::dynamic_array<Il2CppMonoAllocation> Il2cppMonoAllocationFuncVec;
    static Il2cppMonoAllocationFuncVec s_il2cppMonoAllocationFuncs;

    void Profiler::Allocation(Il2CppObject *obj, Il2CppClass *klass, size_t size)
    {
        for (Il2cppMonoAllocationFuncVec::iterator iter = s_il2cppMonoAllocationFuncs.begin(); iter != s_il2cppMonoAllocationFuncs.end(); iter++)
        {
            if ((*iter))
            {
                (*iter)(obj, klass, size);
    ...
    void Profiler::AddAllocTracer(Il2CppMonoAllocation func)
    {
        s_il2cppMonoAllocationFuncs.push_back(func);
    }

    void Profiler::RemoveAllocTracer(Il2CppMonoAllocation func)
    {
        for (Il2cppMonoAllocationFuncVec::iterator iter = s_il2cppMonoAllocationFuncs.begin(); iter != s_il2cppMonoAllocationFuncs.end(); iter++)
        {
            if ((*iter) == func)
            {
                s_il2cppMonoAllocationFuncs.erase(iter);
                break;
            }
        }
    }
    ...
    void Profiler::InstallGCRootUnregister(Il2CppRegisterRootCallback callback)
    {
        if (!s_profilers.size())
            return;
