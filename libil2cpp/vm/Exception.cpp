...
namespace vm
{
    void Exception::PrepareExceptionForThrow(Il2CppException* ex, MethodInfo* lastManagedFrame)
    {
        ...
        // 王者荣耀项目使用了虚拟机技术，对函数的原型进行了修改，导致异常发生时，il2cpp无法找到函数名，无法输出异常调用堆栈
        // 应对方案：直接输出原始的 unwind 异常调用栈，结合符合表工具还原函数
#if IL2CPP_ENABLE_STACKTRACE_PC_ADDRESS
        if (ex->trace_ips == NULL) 
        {
			const StackFrames& frames = *StackTrace::GetStackFramesWithLog();
            
            Il2CppArray* ips = NULL;
            Il2CppArray* raw_ips = NULL;
            Il2CppArray* native_stack_frames = NULL;
            ...
            else 
            {
                ...
                native_stack_frames = Array::New(il2cpp_defaults.stack_frame_class, numberOfFrames);
                for (size_t frame = 0; frame != frames.size() && i >= 0; ++frame, --i)
                {
                    ...
                    if (stackFrameInfo.image_name != NULL && strlen(stackFrameInfo.image_name) != 0)
                    {
                        IL2CPP_OBJECT_SETREF(sf, filename, vm::String::New(stackFrameInfo.image_name));
                    }
                    
                    sf->il_offset = stackFrameInfo.raw_ip;
                    il2cpp_array_setref(native_stack_frames, i, sf);
                }
            }
            ...
            if (native_stack_frames != NULL) 
                IL2CPP_OBJECT_SETREF(ex, _data, native_stack_frames);
        }
#else
...
#endif // IL2CPP_ENABLE_STACKTRACE_PC_ADDRESS
