...
namespace os
{
    ...
    void StackTrace::WalkStackAllInOnce(WalkStackCallbackAllInOnce callback, void* context, WalkOrder walkOrder)
    {
        if (s_StackBacktraceFunc == 0)
        {
            StackTrace::WalkStackNativeAllInOnce(callback, context, walkOrder);
            return;
        }
        ...
        Il2CppMethodPointer addrs[kMaxStackFrames];
        size_t size = s_StackBacktraceFunc(addrs, kMaxStackFrames);
        callback(addrs, size, context, walkOrder);
    }
