...
namespace Image
{
    ...
    static size_t s_TextSectionLength = 0;
    ...
    static size_t InitializeTextSectionLength()
    {
        int imageIndex = GetImageIndex();
        const struct mach_header_64* header = (mach_header_64*)_dyld_get_image_header(imageIndex);
        size_t size;
        const struct segment_command *segmentCmd = (const struct segment_command *)getsegmentdata(header, "__TEXT", &size);
        if (segmentCmd) 
        {
            s_TextSectionLength = size;
        }
    }

    size_t GetTextSectionLength()
    {
        return s_TextSectionLength;
    }
