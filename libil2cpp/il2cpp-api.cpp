...
bool g_class_full_setup_mode = false;
...
int il2cpp_init(const char* domain_name, bool class_full_setup_mode)
{
	g_class_full_setup_mode = class_full_setup_mode;
    INIT_GCDUMP
...
int il2cpp_init_utf16(const Il2CppChar* domain_name, bool class_full_setup_mode)
{
    return il2cpp_init(il2cpp::utils::StringUtils::Utf16ToUtf8(domain_name).c_str(), class_full_setup_mode);
}
...
{
    //old bitmap max 32bit, 64bit is compatibility with 32bit
    size_t dummy = 0;
    Class::GetBitmap(klass, (DWord*)bitmap, dummy);
}
...
void il2cpp_gc_dump_mem(const char* file_path, bool isPrintLargeAlloc)
{
    DUMPGCMEMINFO(file_path, isPrintLargeAlloc);
}

void il2cpp_gc_set_monitor_alloc(bool isActive)
{
	SETMONITORALLOC(isActive);
}

void il2cpp_set_parse_symbol_from_local(bool parse_local)
{
    il2cpp::vm::StackTrace::SetParseFromLocalFlag(parse_local);
}
void il2cpp_begin_trace_allocation(Il2CppMonoAllocation func)
{
    Profiler::AddAllocTracer(func);
}
void il2cpp_end_trace_allocation(Il2CppMonoAllocation func)
{
    Profiler::RemoveAllocTracer(func);
}
bool il2cpp_insight_gc_stacktrace_is_enable()
{
    return il2cpp::vm::InsightGCStackTrace::IsStackTraceEnable();
}

void il2cpp_insight_gc_stacktrace_open(bool open)
{
    if (open)
    {
        il2cpp::vm::InsightGCStackTrace::OpenStackTrace();
    }
    else
    {
        il2cpp::vm::InsightGCStackTrace::CloseStackTrace();
    }
}

int il2cpp_insight_get_gc_stacktrace(unsigned long long* stack, short* module, int count)
{
    return il2cpp::vm::InsightGCStackTrace::GetStackTrace(stack, module, count);
}
