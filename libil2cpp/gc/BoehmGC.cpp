...
#if !RUNTIME_TINY && !IL2CPP_ENABLE_WRITE_BARRIER_VALIDATION && ENABLE_GC_VECTOR_MALLOC
    GC_init_gcj_vector(VECTOR_PROC_INDEX, (void*)GC_gcj_vector_proc);
#endif
	
	GC_set_array_info(
        vm::MetadataCache::GetDescTablePointer(),
        vm::MetadataCache::GetDescTableLength(),
        kIl2CppSizeOfArray,
        kIl2CppOffsetOfArrayLength,
        kIl2CppOffsetElementClass,
        kIl2CppOffsetElementSize);
...
il2cpp::gc::GarbageCollector::FinalizerCallback il2cpp::gc::GarbageCollector::RegisterFinalizerWithCallback(Il2CppObject* obj, FinalizerCallback callback)
{
    ...
    void* oldData;
    REGISTERFINALIZERFORMALLOC(callback);
...
void*
il2cpp::gc::GarbageCollector::MakeDescriptorForObject(DWord *bitmap, int numbits)
{
#ifdef GC_GCJ_SUPPORT
    ...
    //else
    //{
        //GC_descr desc = GC_make_descriptor((GC_word)bitmap, numbits);
        ...
        //return (void*)desc;
    //}
    return (void*)GC_make_descriptor((GC_fword*)bitmap, numbits);
#else
    return 0;
#endif
}
...
{
    #if GC_GCJ_SUPPORT
    return (void*)GC_make_sarray_descriptor();
    #else
    return GC_NO_DESCRIPTOR;
    #endif
}
...
void*
il2cpp::gc::GarbageCollector::AllocateStaticFixed(size_t size, void* descr)
{
    void* tempPtr = GC_MALLOC_STATIC_UNCOLLECTABLE(size, (size_t)descr);
    BIND_MALLOC(tempPtr, size, false);
    return tempPtr;
}
...
{
    RUNFINALIZERFORMALLOC(addr, NULL);
    GC_FREE(addr);
    // Il2Cpp never calls BoehmGC::UnregisterRoot, but we need to call Profiler::UnregisterRoot
